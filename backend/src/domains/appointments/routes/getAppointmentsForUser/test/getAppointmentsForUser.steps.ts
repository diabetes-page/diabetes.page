...
Given(/^There is exactly (\d+) appointment assigned to me$/, async function () {
...
Then(/^the amount of appointments is (\d+)$/, function () {
  expect(this.response.body.appointments.count).equals(1);
});
